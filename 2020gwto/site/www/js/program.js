...
$(function() {
    ...
    $(".imageZoomView .zoomBtn a").on("click", function(e) {
        e.preventDefault();
...
function navi(name, lat, lng) {
    Kakao.init("e70e41e80abc1ac1fcdaa12abbaeecfa"), Kakao.Navi.share({
        name: name,
        x: lng,
        y: lat,
        coordType: "wgs84"
...
function codeReplace(id) {
    return $("#" + id).html().replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/<br>/gi, "\r\n");
}

function copyToClipboard(elementId) {
    var aux = document.createElement("textarea"), code = codeReplace(elementId);
    aux.value = code, document.body.appendChild(aux), aux.select(), document.execCommand("copy"),
        document.body.removeChild(aux);
}

$(function() {
    isMobile.any() && $("body").addClass("mobile");
});
...
function CntCalendar() {
    this._assignElement(), this.init(), this._bindEvent();
}
...
Dropdown.prototype.toggle = function(e) {
    ...
    $(document).ready(function() {
        cntCalendar = new CntCalendar();
    }), CntCalendar.prototype = {
    _assignElement: function() {
        this.dateLi = $(".date > li"), this.statBtn = $(".statBtn"), this.statCnt = $(".statCnt"),
            this.unbindButton = this.dateLi.find(".closeBtn");
    },
    _bindEvent: function() {
        var currentLi, row, rowMod, currentHeight, self = this;
        $(window).on("resize.calendar", this.init.bind(this)), this.statBtn.on("click", function() {
            currentLi = $(this).parent().parent("li").index() + 1, row = currentLi / 7, row = parseInt(row),
                rowMod = currentLi % 7, $(this).parent().parent().height(), currentHeight = $(this).parent().siblings(this.statCnt).innerHeight(),
            0 != rowMod && (row += 1);
            var s = self.calendarRow[row - 1][0], e = self.calendarRow[row - 1][1];
            self.defaultHeight = self.calendarListMaxHeight[row - 1][0];
            for (var i = 0; i < self.calendarRow.length; i++) for (var s2 = self.calendarRow[i][0], e2 = self.calendarRow[i][1], j = s2; j <= e2; j++) self.dateLi.eq(j).css({
                height: self.calendarListMaxHeight[i][0]
            }), self.dateLi.eq(j).find(".date_wrap").css({
                height: self.calendarListMaxHeight[i][1]
            });
            for (i = s; i <= e; i++) self.dateLi.eq(i).css({
                height: self.defaultHeight + currentHeight
            });
            $(this).closest("li").hasClass("active") || (self.dateLi.removeClass("active"),
                $(this).closest("li").addClass("active"), self.statCnt.fadeOut(100), $(this).parent().siblings(self.statCnt).slideDown(200));
        }), this.unbindButton.on("click", function() {
            self.dateLi.removeClass("active");
            for (var i = 0; i < self.calendarRow.length; i++) for (var s2 = self.calendarRow[i][0], e2 = self.calendarRow[i][1], j = s2; j <= e2; j++) self.dateLi.eq(j).css({
                height: self.calendarListMaxHeight[i][0]
            }), self.dateLi.eq(j).find(".date_wrap").css({
                height: self.calendarListMaxHeight[i][1]
            });
            $(this).parent().slideUp(200), $(this).parent().siblings().find(".statBtn").focus();
        });
    },
    init: function() {
        var self = this;
        this.calendarRow = [ [ 0, 6 ], [ 7, 13 ], [ 14, 20 ], [ 21, 27 ], [ 28, 34 ], [ 35, 41 ] ],
            this.calendarListHeight = [], this.calendarListMaxHeight = [], this.defaultHeight = this.dateLi.height(),
            this.dateLi.css({
                height: "auto"
            }), this.dateLi.find("date_wrap").css({
            height: "auto"
        }), $.each(this.dateLi, function(index, item) {
            var itemHeight = $(item).height(), itemWrap = $(item).find(".date_wrap").height();
            self.calendarListHeight.push([ itemHeight, itemWrap ]);
        });
        for (var i = 0; i < this.calendarRow.length; i++) {
            for (var s = this.calendarRow[i][0], e = this.calendarRow[i][1], h = 0, w = 0, j = s; j <= e; j++) void 0 !== this.calendarListHeight[j] && this.calendarListHeight[j][0] > h && (h = this.calendarListHeight[j][0],
                w = this.calendarListHeight[j][1]);
            this.calendarListMaxHeight.push([ h, w ]);
            for (j = s; j <= e; j++) this.dateLi.eq(j).css({
                height: h
            }), this.dateLi.eq(j).find(".date_wrap").css({
                height: w
            });
        }
    }
...
$(function() {
    ...
    CreateMap.DEFAULTS = {
        ...
        var imageSize = new kakao.maps.Size(40, 45), imageOption = {
            offset: new kakao.maps.Point(15, 41)
